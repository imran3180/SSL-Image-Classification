...
def train(device, model, logsoftmax, nll, epoch, train_supervised_dataset, optimizer, batch_size):
	...
	training_loss = 0
	correct = 0
	train_supervised_loader = torch.utils.data.DataLoader(train_supervised_dataset, batch_size = batch_size, shuffle=True, num_workers=1)
...
def label_addition(device, model, softmax, train_supervised_dataset, train_unsupervised_loader, tau):
	model.eval()
	with torch.no_grad():
		...
		for batch_idx, (data, _) in tqdm(enumerate(train_unsupervised_loader)):
			...
			selected_index = (scores > tau).squeeze().nonzero().squeeze()
			new_data = torch.index_select(data, 0, selected_index)
			new_data_label = torch.index_select(indices, 0, selected_index).view(-1)
		...
		return ConcatDataset(train_supervised_dataset, proxy_dataset)
